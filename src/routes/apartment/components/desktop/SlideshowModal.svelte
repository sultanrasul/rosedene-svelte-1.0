<script>
    ...
    export let images = apartmentDetails.amountOfPictures;
    export let apartmentNumber;
    export let apartmentName = apartmentDetails.name;
    /**
  ...
  >
    ...
    <div
      ...
      <div class="flex justify-between items-center px-6 py-4  backdrop-blur-md z-20">
        <h2 class="text-xl font-semibold text-white-800">{apartmentName}</h2>
        <!-- Close Button -->
      ...
      <!-- Bottom Thumbnails -->
      <!-- <div class="flex justify-center items-center gap-2 px-4 py-3 bg-white/90 backdrop-blur-md overflow-x-auto">
        {#each Array.from({ length: images }, (_, i) => i) as i}
          <img
            src={`/${apartmentNumber}/${i}.jpg`}
            alt={`Thumb ${i + 1}`}
            on:click={() => goToSlide(i)}
            class="w-16 h-16 object-cover rounded-md border-2 cursor-pointer transition duration-200 hover:opacity-80"
          />
        {/each}
      </div> -->
